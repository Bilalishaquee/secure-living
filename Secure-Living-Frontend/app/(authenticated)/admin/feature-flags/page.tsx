"use client";

import { useEffect, useState } from "react";
import { Plus, ToggleLeft, ToggleRight, Trash2 } from "lucide-react";
import { useAuth } from "@/lib/auth-context";
import { useToast } from "@/lib/toast-context";
import { Button } from "@/components/ui/Button";
import { Card, CardContent } from "@/components/ui/Card";
import { Modal } from "@/components/ui/Modal";

type FeatureFlag = {
  id: string;
  key: string;
  label: string;
  description: string | null;
  scope: "GLOBAL" | "ORGANIZATION";
  organizationId: string | null;
  isEnabled: boolean;
};

const emptyForm = { key: "", label: "", description: "", scope: "GLOBAL" as "GLOBAL" | "ORGANIZATION", organizationId: "", isEnabled: true };

export default function FeatureFlagsAdminPage() {
  const { user } = useAuth();
  const { toast } = useToast();
  const [rows, setRows] = useState<FeatureFlag[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState(emptyForm);
  const [saving, setSaving] = useState(false);

  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/v1/feature-flags`, { headers: { Authorization: `Bearer ${user?.authToken}` } });
      if (res.ok) {
        const j = await res.json();
        setRows(j.data ?? []);
      }
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => { if (user?.authToken) load(); }, [user?.authToken]);

  async function handleCreate() {
    setSaving(true);
    try {
      const res = await fetch(`/api/v1/feature-flags`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${user?.authToken}` },
        body: JSON.stringify({
          key: form.key,
          label: form.label,
          description: form.description || undefined,
          scope: form.scope,
          organizationId: form.scope === "ORGANIZATION" ? form.organizationId : undefined,
          isEnabled: form.isEnabled,
        }),
      });
      if (res.ok) {
        toast("Feature flag saved", "success");
        setModalOpen(false);
        setForm(emptyForm);
        load();
      } else {
        const j = await res.json().catch(() => ({}));
        toast((j as { error?: string }).error ?? "Failed", "error");
      }
    } finally {
      setSaving(false);
    }
  }

  async function handleToggle(flag: FeatureFlag) {
    const res = await fetch(`/api/v1/feature-flags/${flag.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${user?.authToken}` },
      body: JSON.stringify({ isEnabled: !flag.isEnabled }),
    });
    if (res.ok) {
      toast(flag.isEnabled ? "Feature deactivated" : "Feature activated", "success");
      load();
    } else {
      toast("Failed", "error");
    }
  }

  async function handleDelete(id: string) {
    const res = await fetch(`/api/v1/feature-flags/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${user?.authToken}` },
    });
    if (res.ok) {
      toast("Feature flag removed", "success");
      load();
    } else {
      toast("Failed to remove", "error");
    }
  }

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Feature Flags</h1>
          <p className="mt-1 text-sm text-slate-500">
            Mask (hide) or deactivate any platform feature globally or for a single organization.
          </p>
        </div>
        <Button onClick={() => { setForm(emptyForm); setModalOpen(true); }} className="gap-2">
          <Plus className="h-4 w-4" /> Add Flag
        </Button>
      </div>

      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map((i) => <div key={i} className="h-14 animate-pulse rounded-xl bg-slate-100" />)}</div>
      ) : (
        <Card>
          <CardContent className="p-0">
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Key</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Label</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Scope</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Status</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {rows.map((f) => (
                  <tr key={f.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-mono text-xs text-slate-600">{f.key}</td>
                    <td className="px-4 py-3 text-slate-800">
                      {f.label}
                      {f.description && <p className="text-xs text-slate-400">{f.description}</p>}
                    </td>
                    <td className="px-4 py-3 text-slate-600">{f.scope === "GLOBAL" ? "Platform-wide" : `Org ${f.organizationId?.slice(0, 8)}…`}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${f.isEnabled ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
                        {f.isEnabled ? "Enabled" : "Disabled"}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => handleToggle(f)}
                          className={f.isEnabled ? "text-green-500 hover:text-slate-400" : "text-slate-400 hover:text-green-500"}
                          title={f.isEnabled ? "Deactivate" : "Activate"}
                        >
                          {f.isEnabled ? <ToggleRight className="h-5 w-5" /> : <ToggleLeft className="h-5 w-5" />}
                        </button>
                        <button type="button" onClick={() => handleDelete(f.id)} className="text-slate-400 hover:text-red-500" title="Remove">
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-12 text-center text-slate-500">No feature flags configured yet</td>
                  </tr>
                )}
              </tbody>
            </table>
          </CardContent>
        </Card>
      )}

      <Modal open={modalOpen} onOpenChange={setModalOpen} title="Add Feature Flag">
        <div className="space-y-4">
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Key *</label>
            <input
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-mono focus:border-blue-500 focus:outline-none"
              placeholder="e.g. qr_access_module"
              value={form.key}
              onChange={(e) => setForm((f) => ({ ...f, key: e.target.value.trim() }))}
            />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Label *</label>
            <input
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
              value={form.label}
              onChange={(e) => setForm((f) => ({ ...f, label: e.target.value }))}
            />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Description</label>
            <textarea
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
              rows={2}
              value={form.description}
              onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
            />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Scope</label>
            <select
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
              value={form.scope}
              onChange={(e) => setForm((f) => ({ ...f, scope: e.target.value as "GLOBAL" | "ORGANIZATION" }))}
            >
              <option value="GLOBAL">Platform-wide</option>
              <option value="ORGANIZATION">Single organization</option>
            </select>
          </div>
          {form.scope === "ORGANIZATION" && (
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-700">Organization ID *</label>
              <input
                className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-mono focus:border-blue-500 focus:outline-none"
                value={form.organizationId}
                onChange={(e) => setForm((f) => ({ ...f, organizationId: e.target.value }))}
              />
            </div>
          )}
          <label className="flex items-center gap-2 text-sm text-slate-700">
            <input type="checkbox" checked={form.isEnabled} onChange={(e) => setForm((f) => ({ ...f, isEnabled: e.target.checked }))} />
            Enabled
          </label>
          <div className="flex gap-3 pt-2">
            <Button variant="ghost" onClick={() => setModalOpen(false)} className="flex-1">Cancel</Button>
            <Button
              onClick={handleCreate}
              disabled={!form.key || !form.label || (form.scope === "ORGANIZATION" && !form.organizationId) || saving}
              className="flex-1"
            >
              {saving ? "Saving…" : "Save"}
            </Button>
          </div>
        </div>
      </Modal>
    </div>
  );
}
