...
export const POST = withErrorHandler(async (req: Request, { params }: Ctx) => {
  ...
  if (!canSrTransition(existing.srStatus, SrStatus.CANCELLED)) {
    return jsonError(409, `Cannot cancel a request in status ${existing.srStatus}`);
  }
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const sr = await tx.serviceRequest.update({
      where: { id: params.id },
      data: {
        srStatus: SrStatus.CANCELLED,
        status: "CANCELLED",
        rejectionReason: parsed.data.reason,
        closedAt: now,
      },
    });
    await writeSrTransition(tx, params.id, actor.userId, existing.srStatus, SrStatus.CANCELLED, parsed.data.reason);
    await writeOutboxEvent(tx, SR_EVENT_MAP[SrStatus.CANCELLED], { serviceRequestId: params.id, actorId: actor.userId, reason: parsed.data.reason }, params.id);
    return sr;
  });
