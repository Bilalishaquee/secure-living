...
// Transitions an APPROVED service request into QUOTING status so providers can submit quotes.
export const POST = withErrorHandler(async (req: Request, { params }: Ctx) => {
  ...
  if (!canSrTransition(existing.srStatus, SrStatus.QUOTING)) {
    return jsonError(409, "Cannot request quote from current status " + existing.srStatus);
  }

  const updated = await prisma.$transaction(async (tx) => {
    const sr = await tx.serviceRequest.update({
      where: { id: params.id },
      data: { srStatus: SrStatus.QUOTING, status: SrStatus.QUOTING },
    });
    await writeSrTransition(tx, params.id, actor.userId, existing.srStatus, SrStatus.QUOTING);
    await writeOutboxEvent(tx, SR_EVENT_MAP[SrStatus.QUOTING], { serviceRequestId: params.id, actorId: actor.userId }, params.id);
    return sr;
  });
  ...
  await appendAudit({
    userId: actor.userId,
    role: actor.role,
    action: "service_request.quote_requested",
