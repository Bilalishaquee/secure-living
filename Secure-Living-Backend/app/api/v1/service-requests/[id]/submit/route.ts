...
export const POST = withErrorHandler(async (req: Request, { params }: Ctx) => {
  ...
  if (existing.srStatus === SrStatus.SUBMITTED) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    await writeSrTransition(tx, params.id, actor.userId, existing.srStatus, SrStatus.SUBMITTED);
    await writeOutboxEvent(tx, SR_EVENT_MAP[SrStatus.SUBMITTED], { serviceRequestId: params.id, actorId: actor.userId }, params.id);
    return sr;
  });
